...
func TestBuildInfoHumanStringShape(t *testing.T) {
	cases := []struct {
		...
		{"full", BuildInfo{Version: "v1.2.3", Commit: "abc123", Time: "2026-05-01T00:00:00Z", Go: "go1.25.3"},
			"v1.2.3 (abc123, go1.25.3, 2026-05-01T00:00:00Z)"},
		// Local dev build with no embedded VCS info: commit/time blank,
		// just version + go.
		{"go only", BuildInfo{Version: "dev", Go: "go1.25.3"}, "dev (go1.25.3)"},
		// Truly bare BuildInfo (debug.ReadBuildInfo failed): version alone.
		{"version only", BuildInfo{Version: "v1.2.3"}, "v1.2.3"},
	}
	for _, c := range cases {
...
func TestVersionSubcommandJSONShape(t *testing.T) {
	...
	// are filled lazily so a pinned BuildInfo doesn't need to spoof
	// them — assert they're present after the run rather than checking
	// exact values.
	cfg, stdout, _ := useFake(t, &fakeCA{})
	cfg.JSON = true
	cfg.Build = BuildInfo{Version: "v9.9.9", Commit: "deadbeef", Time: "2026-01-01T00:00:00Z"}
...
func TestVersionSubcommandHumanModeOutput(t *testing.T) {
	...
	cfg, stdout, _ := useFake(t, &fakeCA{})
	cfg.Build = BuildInfo{Version: "v1.0.0", Commit: "abc123", Go: "go1.25.3"}
	cmd := newVersionCmd(cfg)
	if err := cmd.RunE(cmd, nil); err != nil {
