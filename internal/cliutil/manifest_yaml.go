...
// ManifestYAMLFor reconstructs (from cob-provenance.json) or infers
// (ca:// self-references) the manifest YAML for a resolved version.
// Shared by `cob manifest <coords>` (which prints it to stdout) and
// `cob pull <coords>` (which writes it next to the pulled assets so a
// downstream re-publish has something to feed cob).
//
// Lives in cliutil because the two consumers are in distinct packages
// (internal/cli's manifest_cmd, internal/cli/pull) and neither owns the
// helper; cliutil is the shared "cob-aware utilities" surface.
func ManifestYAMLFor(ctx context.Context, client *cob.Client, coords *cob.PackageCoordinates) (string, error) {
	...
	return renderManifestYAML(coords, prov, assets)
}
...
// renderManifestYAML builds the manifest YAML. Provenance mode (prov has
// assets) is a faithful pinned reconstruction; otherwise assets are inferred
// as ca:// self-references. The document is assembled as a yaml.Node and
// encoded, so every key and URI is correctly quoted — a value containing #,
// *, a leading [, a newline, etc. round-trips instead of corrupting the
// output or injecting a line. Errors if there are no sources. Pure (no
// I/O), so it is unit-tested directly.
func renderManifestYAML(c *cob.PackageCoordinates, prov *cob.Provenance, assets []cob.AssetSummary) (string, error) {
